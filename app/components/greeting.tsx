...
import WrongGif from "../../assets/qoobee-no-no-nonono.gif";
...
const phrases = [
  "いえ！きらいでしょう。",
  "やめて！！(đừng mà em ... anh nghĩ em đang hong biết đọc chữ phải honggg ;-;)",
  "君は”いえ”と話したなら私が寂しいですか！！(em nghĩ em nói hong thì anh sẽ bị tổn thương hẻ ?)",
  "寂しすぎました！！(đừng mà em ... anh tổn thương thật;-;)",
  "本当に？(thật lun á hã ;-;,hãy nghĩ lại đi mò)",
  "何で君は私に大好きと話してしまった！！(sao em bảo em thích anh ??? :< !!!!)",
  "’はい！！’と話してください！！（bấm có đi màaa）",
  "”もうあなたを愛していませ”　oa oaaa :< em hết thương anh ròiiii",
  "anh khóc đóooo",
  "đừng bấm nữa =))) bấm sao thì cũng không có いえ　được đâu mà , ehe baka",
];
...
function Greeting({
  ...
  const smallHeartsRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    if (greetingRef.current) {
      gsap.fromTo(
        greetingRef.current.querySelectorAll(".s"),
        { opacity: 0, y: 20 },
        {
          duration: 0.8,
          opacity: 1,
  ...
  useEffect(() => {
    if (smallHeartsRef.current) {
      gsap.fromTo(
        Array.from(smallHeartsRef.current.children),
        { scale: 0, opacity: 0 }, // Starting values
        {
          // Ending values
          duration: 0.5,
          opacity: 1,
  ...
  useEffect(() => {
    // Trigger additional animations for showGreetingMore if needed
    if (greetingRef.current) {
      gsap.fromTo(
        greetingRef.current.querySelectorAll(".s"),
        { opacity: 0, y: 20 },
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setShowContent(true);
    }, 6000); // Set to show content after 5 seconds
  ...
  return (
    <main className="body ">
      <div className="drawBox">
        <div className="smallHearts" ref={smallHeartsRef}>
          <div className="smallHeart"></div>
          <div className="smallHeart"></div>
          <div className="smallHeart"></div>
          <div className="smallHeart"></div>
          <div className="smallHeart"></div>
        ...
        <div ref={greetingRef}>
          <div className="greeting flex">
            <div>
              <span className="s text-red-400">H</span>
              <span className="s text-red-400">a</span>
              <span className="s text-red-400">p</span>
              <span className="s text-red-400">p</span>
              <span className="s text-red-400">y</span>
              <span className="s text-red-400 space">V</span>
              <span className="s text-red-400">a</span>
              <span className="s text-red-400">l</span>
              <span className="s text-red-400">e</span>
              <span className="s text-red-400">n</span>
              <span className="s text-red-400">t</span>
              <span className="s text-red-400">i</span>
              <span className="s text-red-400">n</span>
              <span className="s text-red-400">e</span>
            </div>
          </div>
          ...
          <div className="greeting show">
            <div>
              <span className="s text-red-400">恋</span>
              <span className="s text-red-400">人</span>
              <span className="s space text-red-400"></span>
            </div>
          </div>
        </div>
      </div>
      <div className="mt-[60px] w-full flex justify-center">
        {showContent &&
          (isYes ? (
            <div className="space-y-3 mt-5 w-[360px]">
              <div className="font text-xl text-neutral-500">mật khẩuuuuu</div>
              <div className=" flex items-center ">
                <input
                  className="p-2 w-3/4 border-black"
                  type="text"
                  value={inputPassword}
                  onKeyDown={(e) => {
                    if (e.code === "Enter") {
                ...
                <button
                  ...
                  className="bg-red-500 w-1/4 py-2 px-2 text-white font "
                >
                  Tới luôn
                </button>
              </div>
              {inputError && (
                <div className="space-x-3 flex items-center font">
                  <Image
                    src={WrongGif}
                    alt="wronggg"
                    className="h-10 w-10"
            ...
            <div className="flex justify-center items-center gap-3 mt-5 mb-10 flex-col">
              <div className="w-full text-center text-xl text-neutral-500 font">
                君がもうみたいですか？
              </div>
              <div className="flex gap-x-5 items-center">
                <button
                  ...
                  className="p-2 rounded-md bg-green-600 min-h-[40px] text-[#DEF7EC] hover:opacity-80"
                >
                  はい、私が大好きです！！
                </button>
                <button
                  onClick={handleClickNo}
                  className="p-2 rounded-md w-auto h-auto bg-red-600 text-base text-[#FDE8E8] hover:opacity-80"
                >
                  {getNoButtonText()}
                </button>
              </div>
